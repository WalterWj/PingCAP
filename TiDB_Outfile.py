...
def main():
    ...
    for list_id in range(0, len(id_list), batch):
        ...
        if len(filter_content) == 1:
            filter_content = filter_content[0][0]
            _sql = 'select {} from {} where {} = {}'.format(
                ', '.join(fieldnames), args.table, args.field, filter_content)
        else:
            filter_content = str(tuple([i[0] for i in filter_content]))
            _sql = 'select {} from {} where {} in {}'.format(
                ', '.join(fieldnames), args.table, args.field, filter_content)
    ...
    for num in range(thread):
        ...
        t = threading.Thread(target=outfile_tidb,
                             args=('csv', file_name, fieldnames, queue))
        threads.append(t)
...
def parser_id():
    args = parse_args()
    try:
        schema = mysql_execute(_sql="desc {};".format(args.table))
        id_list = mysql_execute(_sql="select {} as id from {} {};".format(
...
if __name__ == "__main__":
    ...
    main()
    end_time = time.time()
    print('Exiting Main Thread, Total cost time is {}'.format(
        end_time - start_time))
